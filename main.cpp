...
int main()
{
    ...
    std::cout << "Press any key to start.\nMake sure to resize the window to your liking, but the width of the buffer and window must stay the same.\nOnce it starts, you can press escape to end the program.";

    if (getch() != VK_ESCAPE)
    {
        // telling the object to load values from the console window, if not then from the file "data.txt", and if the file cannot be read, the default values will be used instead
        ConsoleDisplay
            targetConsole(ReadBufferSizeFromWindow());

        class Rectangle
            targetRectangle(targetConsole.bufferWidth(), targetConsole.bufferHeight());
        ...
        while (1)
        {
            ...
            */
            ...
            if (clock() - startTime >= frameDuration)
            {
                ...
                updated = targetRectangle.UpdateVectors(0, 0, targetConsole.bufferWidth(), targetConsole.bufferHeight());
            }
